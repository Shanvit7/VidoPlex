...
const handler=async(req, res)=>{
    try{
     ...
     const response = await collection.updateOne({email:payload.email},{ $push: {likedTitles:addedVideo},});
     if(response){
        return res.status(200).json({result:'success',message:'Added to liked titles'});
     } else{
